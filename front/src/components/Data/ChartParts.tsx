...
type AnyScaleType = keyof ScaleTypes;
...
export const ChartParts = <xType extends Scales, yType extends Scales>({
  series,
  xScale,
  yScale,
  hasLine = true,
  ...
  const dataSortedByX = series.map(sortByX);
  const dataSortedByY = series.map(sortByX);
  ...
  useEffect(() => {
    ...
    svg
      ...
      .call(axisBottom(x as d3.AxisScale<number>));
  }, [height, x, ref]);
  ...
  }, [adjustedData, colors, series, hasDots, hasLine, height, x, y, ref]);
  ...
  return (
    <>
      ...
      <div>
        <LegendLabels
          labels={adjustedData.map((datum, index) => ({
            text: datum.legend,
...
type DefinedData = Data<Scales, Scales>;
type DefinedSeries = { data: DefinedData[]; legend?: string };

export const sortByX = ({ data, ...rest }: DefinedSeries): DefinedSeries => ({
  data: data.slice().sort((a, b) => (a.x < b.x ? -1 : 0)),
  ...rest,
});

export const sortByY = ({ data, ...rest }: DefinedSeries): DefinedSeries => ({
  ...
  return (
    <>
      {labels.map(({ text, color }, index) => (
        <span style={{ padding: 10, color }} key={index}>
          {text}
        </span>
      ))}
    </>
...
// const Axes = ({svgRef}: {svgRef: Ref<SVGGElement>, x: }) => {
//   useEffect(() => {
